...
std::string pshe (std::string str)
{
    ...
    std::string string;
        ...
        while (i<str.length())
        {
           ...
           while (((48<=str[i])&&(str[i]<=57))||(str[i]==46)) // проверка на числа (46- точка для десятичный)
           {
           ...
           if (str[i]==40) //провверка на открывающию скобку
           {
               stack.push(str[i]);
               i++;
           }
...
double calculate_RPN (std::string RPN)
{
    ...
    while (i<RPN.length())
    {
        if ( ('0'<=RPN[i]) && (RPN[i]<='9') )
        {
            while ( ( ('0'<=RPN[i]) && (RPN[i]<='9') ) || (RPN[i]=='.') ) // проверка на числа (46- точка для десятичный)
            {
